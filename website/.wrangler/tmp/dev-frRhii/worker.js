...
app.post("/api/installs", async (c) => {
  ...
  for (const pkg of body.packages) {
    ...
    const row = await db.prepare("SELECT id FROM packages WHERE source = ?1 AND name = ?2").bind(body.source, pkg.name).first();
    if (row) {
      await db.prepare("INSERT INTO installs (package_id) VALUES (?1)").bind(row.id).run();
    }
    recorded++;
  }
...
// .wrangler/tmp/bundle-IMRFFB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-IMRFFB/middleware-loader.entry.ts
