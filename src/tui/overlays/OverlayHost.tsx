...
export function OverlayHost({ width, height }: { width: number; height: number }) {
  ...
  let overlayW: number;
  let overlayH: number | undefined; // undefined = auto-size
  ...
  // Max visible items for picker (account for border, title, hint, scroll indicators)
  ...
  const renderOverlay = () => {
    ...
    switch (activeOverlay) {
      case "picker":
        return (
          <Picker
            key={(props.title as string) ?? "picker"}
            title={(props.title as string) ?? "Select"}
            items={(props.items as Array<{ label: string; value: string }>) ?? []}
            onSelect={(index, value) => {
              // CRITICAL: close FIRST, then callback — otherwise callback's openOverlay
              // gets immediately closed by our closeOverlay call
              closeOverlay();
      ...
      case "task-browser":
        return (
          <ContentViewer
            key={(props.title as string) ?? "viewer"}
  ...
  return (
    <Box
      ...
      alignItems="center"
    >
      <Box width={overlayW} height={overlayH}>
        {renderOverlay()}
      </Box>
    </Box>
  );
