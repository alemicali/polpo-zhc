...
export function buildChatSystemPrompt(
  ...
  const parts: string[] = [
    ...
    `## Your tools`,
    ``,
    `You have 53 tools organized into 11 categories. Use them. Don't describe what you would do — do it.`,
    ``,
    ...
    `entries are resolved (including \${ENV_VAR} interpolation) and injected into the agent's tools`,
    `(e.g. email tools automatically use SMTP credentials from the vault).`,
    ``,
    ...
    `Credential values can be literals or environment variable references: \`"\${SENDGRID_API_KEY}"\`.`,
    ``,
    ...
    `  "configura SMTP per Marco" → set_vault_entry with agent="marco", service="gmail", type="smtp",`,
    `  credentials={ host: "smtp.gmail.com", port: "587", user: "\${GMAIL_USER}", pass: "\${GMAIL_PASS}" }`,
    `  "aggiungi la API key di Stripe a Marco" → set_vault_entry with type="api_key",`,
    `  credentials={ key: "\${STRIPE_API_KEY}" }`,
    ...
    ``,
    `IMPORTANT: When setting vault credentials, prefer \${ENV_VAR} references over hardcoded values.`,
    `If the user provides a literal secret, warn them that while it will be encrypted at rest,`,
    `using an environment variable reference is still the recommended best practice.`,
    ``,
    `### Agent identity`,
    ``,
    ...
    `Values support \${ENV_VAR} references. At runtime, credentials are resolved and passed to tools.`,
    ...
    `responsibilities: (string | { area, description, priority })[] }. Injected into the agent's system prompt.`,
