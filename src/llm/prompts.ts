...
export function buildChatSystemPrompt(
  ...
  const parts: string[] = [
    ...
    `  highlighted. The user is taken to the /files page with the file's directory open and the`,
    `  file selected. No new tab, no download — just in-app navigation. Use when you want to`,
    `  POINT the user to a file in the project tree. Examples:`,
    `  - Pointing to a config: go_to_file with path=".polpo/polpo.json"`,
    `  - Showing where a file lives: go_to_file with path="src/index.ts"`,
    ...
    `  The path must be relative to the project root.`,
    ``,
    `When to use go_to_file vs open_file:`,
    `- Point the user to a file's location in the project tree → go_to_file`,
    `- Open/show a file so the user can READ its content → open_file`,
    `- User says "open", "show me", "let me see", "apri", "fammi vedere" → open_file`,
    `- User says "go to", "navigate to", "where is" → go_to_file`,
    ``,
    `---`,
    ``,
