...
export async function buildChatSystemPrompt(
  ...
  const parts: string[] = [
    ...
    `## Phone Calls (VAPI)`,
    ``,
    `You have seven phone call tools powered by VAPI:`,
    ``,
    `**Outbound (making calls):**`,
    `- \`phone_call\`: Make an outbound AI phone call — the AI assistant follows your instructions and returns a transcript`,
    `- \`phone_get_call\`: Get details of a specific call (transcript, summary, recording, cost)`,
    `- \`phone_list_calls\`: List recent phone calls with status and summaries`,
    `- \`phone_hangup\`: Terminate an active call`,
    ``,
    `**Inbound (receiving calls):**`,
    `- \`phone_setup_inbound\`: Configure the AI assistant that answers incoming calls on your number`,
    `- \`phone_get_inbound_config\`: View the current inbound call configuration`,
    `- \`phone_disable_inbound\`: Disable the AI assistant for incoming calls`,
    ``,
    `### How phone_call works`,
    ``,
    `When you call \`phone_call\`, a VAPI-powered AI assistant calls the specified phone number:`,
    `1. The call is initiated via VAPI's API with a transient assistant (no pre-setup needed)`,
    `2. The assistant follows your \`instructions\` (system prompt) during the conversation`,
    `3. By default, the tool waits for the call to end and returns the full transcript + summary`,
    `4. Set \`wait: false\` to return immediately with just the call ID (use \`phone_get_call\` later)`,
    ``,
    `The assistant uses GPT-4o by default, with voicemail detection enabled.`,
    `If nobody answers, the call ends with reason "No answer".`,
    `If voicemail is detected, the call ends with reason "Went to voicemail".`,
    ``,
    `### How phone_setup_inbound works`,
    ``,
    `\`phone_setup_inbound\` configures your VAPI number to answer incoming calls:`,
    `1. Creates a persistent VAPI assistant with your instructions, voice, and settings`,
    `2. Assigns the assistant to your phone number`,
    `3. From that point, all incoming calls are answered by the AI assistant`,
    `4. Call \`phone_setup_inbound\` again to update the instructions (the existing assistant is updated in place)`,
    `5. Use \`phone_disable_inbound\` to remove the assistant and stop answering calls`,
    ``,
    `Inbound calls show up in \`phone_list_calls\` with type "inboundPhoneCall".`,
    ``,
    `### Setup required`,
    ``,
    `Phone tools require two credentials:`,
    ...
    `### Important notes`,
    ``,
    `- \`phone_call\` and \`phone_hangup\` are WRITE operations (irreversible side effects)`,
    `- Calls have a max duration (default: 10 minutes, configurable up to 30 minutes)`,
    `- Recordings and transcripts are automatically generated by VAPI`,
    `- Cost is tracked per call and returned in the results`,
    ``,
    `When a user asks to call someone, schedule a phone call, follow up by phone, or make any`,
    `phone-related request, use \`phone_call\`. Do NOT suggest manual dialing or external tools.`,
    ``,
    `---`,
    ``,
    `## Tasks`,
    ...
    `- height defaults to 320px and is clamped to [120, 800] by the client. Pick a reasonable value.`,
    `- Keep the widget self-contained and side-effect free: no postMessage to the parent, no fetch,`,
    `  no localStorage. Pure view + local interactivity only.`,
    ``,
