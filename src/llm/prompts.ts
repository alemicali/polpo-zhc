...
export function buildChatSystemPrompt(
  ...
  const parts: string[] = [
    ...
    ``,
    `- add_agent: Create a new agent with name, role, model, systemPrompt, skills, allowedPaths.`,
    `  skills: array of skill names the agent can use (e.g. ["browser", "pdf", "excel"]).`,
    `  allowedPaths: filesystem sandbox — restrict which directories the agent can access.`,
    `- update_agent: Modify an existing agent's configuration (model, role, system prompt, skills, paths).`,
    `  "cambia il modello di Marco" → update_agent with name and model.`,
    `  "aggiungi le skill browser a Marco" → update_agent with skills.`,
    `  Can also update the agent's systemPrompt — use this when the user wants to change how a`,
