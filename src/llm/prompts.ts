...
export function buildChatSystemPrompt(
  ...
  const parts: string[] = [
    ...
    ``,
    `**IMPORTANT**: When the user wants to wait a fixed time between tasks (e.g. "aspetta 2 ore prima`,
    `del deploy", "wait 30 minutes after build", "cooldown period"), ALWAYS use native mission delays.`,
    ...
    `### Templates`,
    ``,
    `**Template tools**: list_templates, get_template, create_template, instantiate_template, run_template, delete_template.`,
    `Templates are reusable, parameterized mission definitions. They live on disk as`,
    `\`.polpo/templates/<name>/template.json\` and produce standard missions when instantiated.`,
    ``,
    ...
    `- **Review first**: create_template → instantiate_template (creates draft mission) → user reviews → execute_mission.`,
    `- **Direct run**: create_template → run_template (creates + executes in one step).`,
    ``,
    `**Key rules**:`,
    `- Template names must be kebab-case (e.g. "code-review", "bug-fix", "data-migration").`,
    ...
    `- Once instantiated, the result is a standard Mission — the template is not involved anymore.`,
    `- Templates are discovered from .polpo/templates/ (project) and ~/.polpo/templates/ (user).`,
    ``,
    ...
    `  "manda un messaggio a Marco su WhatsApp" → whatsapp_send with to="Marco", text="...".`,
    `  "scrivi a 393387172954" → whatsapp_send with to="393387172954", text="...".`,
    ...
    `    "mostra le chat WhatsApp" → whatsapp_read with action="list_chats".`,
    `  - \`read_chat\`: Read messages from a specific chat. Requires chatId (phone, name, or JID).`,
    `    "leggi i messaggi di Marco" → whatsapp_read with action="read_chat", chatId="Marco".`,
    `  - \`search\`: Search text across all chats. Requires query.`,
    `    "cerca 'riunione' nei messaggi" → whatsapp_read with action="search", query="riunione".`,
    `  - \`contacts\`: List known contacts from the WhatsApp store.`,
    `    "mostra i contatti WhatsApp" → whatsapp_read with action="contacts".`,
    ``,
    `When to use these yourself vs. delegating to an agent:`,
