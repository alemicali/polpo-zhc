...
export class AgentManager {
  ...
  addAgent(agent: AgentConfig): void {
    ...
    this.ctx.registry.setState({ team: this.ctx.config.team });
    this.ctx.emitter.emit("log", { level: "info", message: `Agent added: ${agent.name} (${agent.adapter})` });
  }
  ...
  addVolatileAgent(agent: AgentConfig, group: string): void {
    ...
    this.ctx.registry.setState({ team: this.ctx.config.team });
    this.ctx.emitter.emit("log", { level: "info", message: `Volatile agent added: ${agent.name} (${agent.adapter}) for ${group}` });
  }
