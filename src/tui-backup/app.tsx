...
function App() {
  ...
  useEffect(() => {
    ...
    store.logAlways(`  ${team.name} (${team.agents.map((a) => a.name).join(", ")})`, [
      S("  ", "gray"),
      S(team.name, "cyan"),
      S(` (${team.agents.map((a) => a.name).join(", ")})`, "gray"),
    ]);
    store.logAlways("", []);
    store.logAlways("Type a task and press Enter. @agent to assign, /help for commands.", [
      S("Type a task and press Enter. @agent to assign, /help for commands.", undefined, false, true),
    ]);
    store.logAlways("");
  ...
  return (
    <Box flexDirection="column" height={height} width={width}>
      ...
      <Box flexDirection="row" height={bodyH} overflow="hidden">
        <Box width={logW} height={bodyH} overflow="hidden">
          <LogPanel width={logW} height={bodyH} />
        </Box>
        {store.taskPanelVisible && (
          <Box width={taskW} height={bodyH} overflow="hidden">
            <TaskPanel width={taskW} height={bodyH} />
          </Box>
        )}
      </Box>
