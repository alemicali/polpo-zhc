...
describe("SqliteRunStore", () => {
  ...
  describe("upsertRun", () => {
    ...
    it("updates existing run on conflict", async () => {
      const run = createTestRun();
      await store.upsertRun(run);

      const updated = createTestRun({ pid: 99999, status: "running" });
      await store.upsertRun(updated);
  ...
  describe("updateActivity", () => {
    it("updates activity for a run", async () => {
      await store.upsertRun(createTestRun());
      ...
      await store.updateActivity("run-1", newActivity);
  ...
  describe("completeRun", () => {
    it("marks a run as completed with result", async () => {
      ...
      await store.completeRun("run-1", "completed", result);
    ...
    it("marks a run as killed", async () => {
      await store.upsertRun(createTestRun());
      await store.completeRun("run-1", "killed", {
        exitCode: 1, stdout: "", stderr: "killed", duration: 100,
      });
      expect((await store.getRun("run-1"))!.status).toBe("killed");
    });
  });

  describe("getRunByTaskId", () => {
    it("finds run by task ID", async () => {
      await store.upsertRun(createTestRun({ id: "run-1", taskId: "task-abc" }));
      const found = await store.getRunByTaskId("task-abc");
      expect(found).toBeDefined();
      expect(found!.id).toBe("run-1");
    });

    it("returns undefined for unknown task", async () => {
      expect(await store.getRunByTaskId("nonexistent")).toBeUndefined();
    });

    it("returns most recent run for a task", async () => {
      await store.upsertRun(createTestRun({
        id: "run-old",
        taskId: "task-1",
      ...
      const found = await store.getRunByTaskId("task-1");
  ...
  describe("getActiveRuns", () => {
    it("returns only running runs", async () => {
      await store.upsertRun(createTestRun({ id: "run-1", status: "running" }));
      await store.upsertRun(createTestRun({ id: "run-2", taskId: "task-2", status: "running" }));
      await store.upsertRun(createTestRun({ id: "run-3", taskId: "task-3", status: "completed" }));
      await store.upsertRun(createTestRun({ id: "run-4", taskId: "task-4", status: "failed" }));
    ...
    it("returns empty array when no running runs", async () => {
      await store.upsertRun(createTestRun({ id: "run-1", status: "completed" }));
      expect(await store.getActiveRuns()).toHaveLength(0);
    });
  });

  describe("getTerminalRuns", () => {
    it("returns completed, failed, and killed runs", async () => {
      await store.upsertRun(createTestRun({ id: "run-1", status: "running" }));
      await store.upsertRun(createTestRun({ id: "run-2", taskId: "task-2", status: "completed" }));
      await store.upsertRun(createTestRun({ id: "run-3", taskId: "task-3", status: "failed" }));
      await store.upsertRun(createTestRun({ id: "run-4", taskId: "task-4", status: "killed" }));

      const terminal = await store.getTerminalRuns();
  ...
  describe("deleteRun", () => {
    ...
    it("no-op for nonexistent run", async () => {
      await expect(store.deleteRun("nonexistent")).resolves.not.toThrow();
    });
  });

  describe("concurrent access", () => {
    it("two stores can access the same DB", async () => {
      const store2 = new SqliteRunStore(DB_PATH);
      try {
        await store.upsertRun(createTestRun({ id: "run-1" }));
        const fromStore2 = await store2.getRun("run-1");
        expect(fromStore2).toBeDefined();
        expect(fromStore2!.id).toBe("run-1");

        await store2.updateActivity("run-1", createTestActivity({ toolCalls: 10 }));
        const fromStore1 = await store.getRun("run-1");
