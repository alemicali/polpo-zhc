...
Transcribe audio to text (STT) and generate speech from text (TTS). Enable with `enableAudio: true` in agent config.
...
- **Text-to-speech** — OpenAI (default), ElevenLabs

**Required environment variables** (depending on provider): `OPENAI_API_KEY`, `DEEPGRAM_API_KEY`, or `ELEVENLABS_API_KEY`.
...
| `provider` | enum: `openai` \| `elevenlabs` | no | TTS provider (default `openai`) |
...
- `speed` is only supported by OpenAI — it is ignored for ElevenLabs
