---
title: "React SDK"
description: "React hooks with real-time SSE updates, built on @lumea-labs/polpo-client"
---

The `@lumea-labs/polpo-react` package provides type-safe React hooks with real-time SSE updates, built on top of [`@lumea-labs/polpo-client`](/reference/client-sdk).

<Note>
For non-React environments (Node.js scripts, Vue, Svelte, server-side), use [`@lumea-labs/polpo-client`](/reference/client-sdk) directly. The React SDK re-exports all client types and classes for convenience.
</Note>
...
This automatically installs `@lumea-labs/polpo-client` as a dependency. Peer dependencies: React 18 or 19.
...
import type { Task, Mission } from "@lumea-labs/polpo-react";
...
All types and classes from `@lumea-labs/polpo-client` are re-exported from `@lumea-labs/polpo-react` for backward compatibility.
...
1. **[`@lumea-labs/polpo-client`](/reference/client-sdk)** — Framework-agnostic HTTP client, SSE manager, reactive store
...
<Note>
The React SDK depends on `@lumea-labs/polpo-client` (auto-installed) and has React as a peer dependency. Both packages are tree-shakeable with `sideEffects: false`.
</Note>
