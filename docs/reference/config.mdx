...
<Note>
**HTTP tools are always-on**: The `http_fetch` and `http_download` tools are included in the core tool set and available to all agents by default — no enable flag needed. Git, multi-file, dependency management, Excel/CSV, PDF, Word/DOCX, audio, and image tool categories have been removed. Use `bash` commands or [skills](/features/skills) for these operations instead.
</Note>
