...
<Note>
All state transitions are **atomic** — the task registry uses file-based stores (default) with atomic write operations to ensure consistency even if the process crashes mid-transition. SQLite and PostgreSQL are available as alternative stores via `@polpo/drizzle`.
</Note>
