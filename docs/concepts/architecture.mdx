...
2. **Core** — the supervisor loop, task registry, mission executor, system context (`.polpo/system-context.md`), memory (shared + per-agent), and all the managers (assessment, approvals, escalation, scheduling, etc.)
...
File-based stores are the default. SQLite and PostgreSQL are available as alternatives via configuration (powered by [Drizzle ORM](https://orm.drizzle.team/) in `@polpo-ai/drizzle`).
