...
Orchestra uses **G-Eval** (LLM-as-judge) to automatically assess completed tasks. Each task's output is evaluated against multiple dimensions, producing scores from 1-5 with chain-of-thought reasoning.
...
When no custom dimensions are specified, Orchestra uses these defaults:
...
Orchestra includes an **expectation judge** that can detect when expectations themselves are wrong (not the agent's work). If the LLM determines that a failing expectation is unreasonable given the task, it can auto-correct the expectations and re-assess.
