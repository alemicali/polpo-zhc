...
Every task in Orchestra moves through a well-defined set of states:
...
When assessment fails, Orchestra first tries a **fix phase** — sending the agent specific feedback about what's wrong so it can make targeted corrections without starting over:
