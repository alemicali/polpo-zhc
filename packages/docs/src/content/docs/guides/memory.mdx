...
Project memory is a markdown file (`.orchestra/memory.md`) that Orchestra injects into every agent's context. It's persistent across sessions and shared by all agents working on your project.
...
Create `.orchestra/memory.md` in your project root:
...
When an agent is spawned for a task, Orchestra:

1. Reads `.orchestra/memory.md`
