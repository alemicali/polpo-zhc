...
The `@orchestra/react` SDK provides type-safe React hooks with real-time SSE updates.

## Installation

```bash
npm install @orchestra/react
```
...
Wrap your app to provide Orchestra context:
...
|------|------|----------|-------------|
| `baseURL` | string | yes | Orchestra server URL |
...
Access the Orchestra client and connection status.
...
1. **OrchestraClient** — HTTP client with automatic request deduplication
2. **EventSourceManager** — SSE with auto-reconnect and exponential backoff
3. **OrchestraStore** — Central state with `reduceEvent()` for 35+ event types
