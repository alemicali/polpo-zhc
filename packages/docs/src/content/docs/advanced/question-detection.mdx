...
Orchestra detects when an agent's output contains a question and auto-generates an answer:

1. **Detection**: After the agent finishes (or during activity polling), Orchestra checks the agent's latest output for questions
...
Orchestra uses two detection approaches:
...
- **Project memory**: Persistent context from `.orchestra/memory.md`
...
Orchestrator generates answer using project context:
