...
Polpo provides seven CLI commands: `init`, `run`, `status`, `serve`, `tui`, `workflow`, and `skills`.
...
Manage and execute reusable [workflow templates](/features/workflows).
...
See [Writing Workflows](/features/writing-workflows) for how to create your own.
...
```bash
polpo skills add <source> [options]
```

| Option | Default | Description |
|--------|---------|-------------|
| `-d, --dir <path>` | `.` | Working directory |
| `-s, --skill <names...>` | — | Install only specific skill names |
| `-g, --global` | `false` | Install to `~/.polpo/skills/` (shared across projects) |
| `-f, --force` | `false` | Overwrite existing skills |
...
# Assign to an agent after install
