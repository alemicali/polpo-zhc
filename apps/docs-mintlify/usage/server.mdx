...
This endpoint accepts standard OpenAI-format messages and returns responses in OpenAI format. Polpo runs its full tool loop internally (37 tools) — you describe what you need, Polpo handles the rest. Supports `"stream": true` for SSE streaming.
...
curl http://localhost:3890/v1/chat/completions \
  ...
  -d '{
    "messages": [{"role": "user", "content": "Create a plan to build a REST API for user management"}],
    "stream": false
  }'
```
