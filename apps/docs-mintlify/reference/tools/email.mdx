...
Send, read, and search emails. Enable with `enableEmail: true` in agent config.
...
<Tip>
The easiest way to configure email credentials is through the **onboarding wizard**: `polpo agent onboard <name>`. Step 4 walks you through SMTP and IMAP setup, creates vault entries with `${ENV_VAR}` references, and sets `enableEmail: true` automatically. See [Onboarding walkthrough](/guides/build-your-team#onboard-your-agents).
</Tip>
