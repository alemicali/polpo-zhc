...
<Note>
Plans and tasks are stored separately in `.polpo/plans/` and `.polpo/tasks/` — they are not part of the config file. Use `polpo plan create`, the TUI, or the API to manage them.
</Note>
...
Tasks are created inside plans (via CLI, TUI, or API). Each task has these fields:
