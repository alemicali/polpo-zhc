...
```tsx
const { plans, isLoading, createPlan, updatePlan, executePlan, resumePlan, abortPlan, deletePlan, refetch } = usePlans();
```
...
| `updatePlan` | `(id, opts) => Promise<Plan>` | Update plan metadata |
...
Single plan with actions and completion report.

```tsx
const { plan, report, isLoading, error, updatePlan, executePlan, resumePlan, abortPlan, deletePlan } = usePlan("plan-1");
```
...
| Field | Type | Description |
|-------|------|-------------|
| `plan` | `Plan \| undefined` | Plan object |
| `report` | `PlanReport \| undefined` | Completion report (populated from `plan:completed` SSE event) |
| `isLoading` | `boolean` | Initial load in progress |
| `error` | `Error \| null` | Fetch error |
| `updatePlan` | `(opts) => Promise<Plan>` | Update plan metadata |
| `executePlan` | `() => Promise<{ tasks: Task[] }>` | Execute the plan |
| `resumePlan` | `(opts?) => Promise` | Resume failed plan |
| `abortPlan` | `() => Promise<{ aborted: number }>` | Abort active plan |
| `deletePlan` | `() => Promise<void>` | Delete plan |
...
```tsx
const { workflows, loading, getWorkflow, runWorkflow, refetch } = useWorkflows();
```
...
| Field | Type | Description |
|-------|------|-------------|
| `workflows` | `WorkflowInfo[]` | Available workflows |
| `loading` | `boolean` | Initial load in progress |
| `getWorkflow` | `(name) => Promise<WorkflowDefinition>` | Fetch full workflow definition |
| `runWorkflow` | `(name, params?) => Promise<WorkflowRunResult>` | Execute a workflow |
| `refetch` | `() => void` | Force refetch |
...
```tsx
// Full import
import { PolpoProvider, useTasks, usePlans } from "@openpolpo/react-sdk";

// Direct client import (no React dependency)
import { PolpoClient } from "@openpolpo/react-sdk/client";

// Individual hook imports
import { useTasks } from "@openpolpo/react-sdk/hooks/use-tasks";
```
