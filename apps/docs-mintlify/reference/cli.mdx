...
4. **Email** — SMTP vault entry (send) and IMAP vault entry (read), sets `enableEmail: true`
5. **Vault** — additional credentials (API keys, OAuth tokens, logins, custom)

All changes are saved to `.polpo/polpo.json`. Run again to update an existing agent's profile.
