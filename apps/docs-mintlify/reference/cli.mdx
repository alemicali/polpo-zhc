...
```bash
polpo workflow list [-d <path>]
```

### workflow show

```bash
polpo workflow show <name> [-d <path>]
```

### workflow run

```bash
polpo workflow run <name> [-d <path>] [-p key=value...] [--dry-run]
```
...
Validate a workflow definition (checks fields, tasks, placeholders, unused params).

```bash
polpo workflow validate <name> [-d <path>]
```
