...
```json
{
  ...
  "vault": {
    "email": {
      "type": "smtp",
      "credentials": { "host": "smtp.gmail.com", "user": "${ALICE_SMTP_USER}", "pass": "${ALICE_SMTP_PASS}" }
    }
  },
  "enableEmail": true,
...
| `vault` | `Record<string, VaultEntry>` | Per-agent credentials (SMTP, IMAP, OAuth, API keys) |
...
This interactive CLI walks through 5 steps — identity, persona (responsibilities + tone + personality), hierarchy, email, and additional vault credentials — and writes everything to `.polpo/polpo.json`.
...
Each agent can have its own set of credentials for different services. Vault entries use `${ENV_VAR}` references to keep secrets out of the config file:

```json
"vault": {
  "email": { "type": "smtp", "credentials": { "user": "${ALICE_SMTP_USER}", "pass": "${ALICE_SMTP_PASS}" } },
  "twitter": { "type": "oauth", "credentials": { "api_key": "${TWITTER_KEY}" } },
  "crm": { "type": "login", "credentials": { "url": "https://crm.example.com", "username": "alice", "password": "${CRM_PASS}" } }
}
```

At runtime, Polpo resolves `${ENV_VAR}` from environment variables and passes the credentials to the agent's tools. Credential resolution order: tool parameters > vault > environment variables.
