...
1. **Interface layer** — CLI, TUI, Web UI, and REST API provide different ways to talk to Polpo
2. **Core** — the supervisor loop, task registry, plan executor, and all the managers (assessment, approvals, escalation, scheduling, etc.)
...
Each agent run produces an `AgentHandle` that the orchestrator uses to monitor the agent:
