...
<Tabs>
  <Tab title="HTTP API">
    ...
    curl http://localhost:3890/api/v1/projects/my-project/plans/checkpoints

    # Resume a checkpoint
    curl -X POST http://localhost:3890/api/v1/projects/my-project/plans/{planId}/checkpoints/review-analysis/resume
    ```
  </Tab>
...
Checkpoints work in [Workflows](/features/workflows) — use `{{placeholders}}` in task titles referenced by `afterTasks` and `blocksTasks`:
