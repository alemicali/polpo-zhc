...
Polpo is **fully provider-agnostic**. Every agent can use a different model from a different provider, the orchestrator runs on its own model, and you can configure fallback chains, cost limits, and custom endpoints — all from a single config file.
...
Anything that doesn't match falls back to the `opencode` provider.
...
The orchestrator uses its own model for plan generation, question detection, deadlock resolution, escalation decisions, and chat mode:
...
When no model is configured anywhere, Polpo uses **`opencode:big-pickle`** — a free model that requires no API key. It's good for trying Polpo out without any setup, but you'll want to switch to a real provider for production work.
