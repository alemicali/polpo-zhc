...
| `vault` | `Record<string, VaultEntry>` | — | Per-agent credentials (SMTP, IMAP, OAuth, API keys). See [Vault](#per-agent-credentials-vault) |
...
| `enableEmail` | `boolean` | `false` | Give the agent email tools (5 tools, requires vault credentials) |
...
4. **Email** — SMTP and IMAP vault configuration.
   ...
   This creates vault entries (type `"smtp"` and `"imap"`) and sets `enableEmail: true`. Use `${ENV_VAR}` syntax for secrets — Polpo resolves them at runtime from environment variables.

   Enabling email gives the agent 5 tools: `email_send`, `email_verify`, `email_list`, `email_read`, `email_search`. See [Email Tools](/reference/tools/email).
...
The wizard saves everything to `.polpo/polpo.json`. Run it again anytime to update an agent's configuration.
...
Each agent can have its own credentials for email, APIs, logins, etc. Secrets use `${ENV_VAR}` references:
...
See [Configuration](/reference/config#vaultentry-fields) for all vault entry types.
