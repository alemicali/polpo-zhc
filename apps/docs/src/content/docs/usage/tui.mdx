...
The Chat tab has three input modes that determine how free-text input (non-slash-command) is handled:

| Mode | Command | Behavior |
|------|---------|----------|
| **Task** | `/task` | Free text creates a new task |
| **Plan** | `/plan` | Free text generates a plan with AI |
| **Chat** | `/chat` | Free text is conversational Q&A with the orchestrator |

The current mode is shown in the input prompt. Switch modes with the corresponding slash command.
