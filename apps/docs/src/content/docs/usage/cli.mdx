...
Polpo provides four CLI commands: `init`, `run`, `status`, `serve`, and `tui`.
...
Creates:
- `polpo.yml` — configuration file with example agents and plan
- `.polpo/` — runtime directory for state, logs, and assessments
...
|--------|---------|-------------|
| `-c, --config <path>` | `.` | Path to working directory containing `polpo.yml` |
...
All commands accept the `--config` / `-c` flag to specify the working directory. Polpo will look for `polpo.yml` in that directory.

```bash
# Run from a different directory
polpo run -c /path/to/my-project

# Serve from a specific project
polpo serve -c ./projects/api-service --port 3890
```
