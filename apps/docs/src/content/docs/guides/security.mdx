...
By default, child processes inherit the full `process.env` of their parent. This means every agent subprocess — including bash tool commands and MCP stdio servers — would have access to all API keys, tokens, and secrets in the environment.

An agent could trivially read secrets via `env`, `echo $SECRET`, or any other shell command.
...
Notification rules support conditions that filter which events trigger notifications. A naive approach would be `eval()` or `new Function()` to evaluate user-provided expressions — this would be a code injection vulnerability.

### The Structured Approach

Instead, Polpo uses a pure-data JSON condition format (`NotificationCondition`) that is evaluated without any string parsing or code execution:
